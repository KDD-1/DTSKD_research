...
============================================================================
对比: clean / noise20 / noise50 各 HistKD vs MCW-AF v2
目标: 验证 "噪声率 ↑ → AF 净收益 ↑" 趋势，确认 AF 首次超越 HistKD 的噪声阈值
"""
...
try:
    all_results['0%'] = analyze_experiment_pair(
        'experiments/forgetting_hist_s27',
        'experiments/forgetting_ours_s27',
        'Clean')
...
try:
    all_results['20%'] = analyze_experiment_pair(
        'experiments/noise20_hist_s27',
        'experiments/noise20_ours_s27',
        '20% Noise')
...
try:
    all_results['50%'] = analyze_experiment_pair(
        'experiments/noise50_hist_s27',
        'experiments/noise50_ours_s27',
        '50% Noise')
...
for key, color in zip(noise_keys, colors_noise):
    ...
    for suffix, label, ls in [
        ...
        if key == '0%':
            exp_dir = f'experiments/forgetting{suffix}'
        else:
            exp_dir = f'experiments/{noise_dir}{suffix}'
        try:
...
for key, color in zip(noise_keys, colors_noise):
    ...
    noise_dir = 'noise' + key.replace('%', '')
    if key == '0%':
        hist_dir = 'experiments/forgetting_hist_s27'
        ours_dir = 'experiments/forgetting_ours_s27'
    else:
