"""Anti-Forgetting SKD 超参数网格搜索
搜索 α (尺度因子), τ (中性阈值), λ (约束强度) 的最优组合
每个组合跑 20 epoch, 快速筛选最佳参数
"""
import subprocess
import os
import sys
...
# 网格: 7 个组合 (默认 + 3 个参数各 2 个变体)
GRID = [
    # (name_suffix, af_alpha, af_tau, af_lambda)
    ("default",     2.0, 0.2, 0.5),   # 默认
    ("alpha_1.0",   1.0, 0.2, 0.5),   # α 低: 温和权重
    ("alpha_4.0",   4.0, 0.2, 0.5),   # α 高: 激进保护
    ("tau_0.1",     2.0, 0.1, 0.5),   # τ 低: 易提权
    ("tau_0.4",     2.0, 0.4, 0.5),   # τ 高: 严格门槛
    ("lambda_0.1",  2.0, 0.2, 0.1),   # λ 低: 弱约束
    ("lambda_1.0",  2.0, 0.2, 1.0),   # λ 高: 强约束
]
...
for name_suffix, alpha, tau, lam in GRID:
    ...
    try:
        ...
        # 读取 CSV 最后一行
        csv_path = os.path.join(WORK_DIR, "experiments", exp_name, "metrics.csv")
        last_csv = None
