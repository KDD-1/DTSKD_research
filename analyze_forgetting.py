...
SKD 遗忘检测分析脚本
============================================================================
基于 Stern et al. (AAAI 2025 / TPAMI 2026) 的 Forget Fraction 度量，
分析训练过程中每个 epoch 的样本级遗忘行为。
...
def load_per_sample_correct(forgetting_dir):
    """加载所有 epoch 的 per-sample correctness 张量
...
def compute_forget_learn(correct_matrix, epochs):
    """计算每个 epoch 的 Forget Fraction 和 Learn Fraction

    Args:
        correct_matrix: (n_epochs, n_samples) — 每个样本在每个 epoch 是否被正确分类
        epochs: (n_epochs,) — epoch 编号

    Returns:
        F: (n_epochs,) Forget Fraction
        L: (n_epochs,) Learn Fraction
        acc: (n_epochs,) 每个 epoch 的验证精度
        final_wrong_mask: (n_samples,) 最终模型分错的样本
    ...
    # F_e = (在 epoch e 正确 且 最终错误 的样本数) / 总样本数
    F = np.zeros(n_epochs)
    L = np.zeros(n_epochs)

    for e in range(n_epochs):
        # 遗忘: epoch e 正确 → 最终错误
        forgotten = correct_matrix[e] & final_wrong_mask
        F[e] = forgotten.sum() / n_samples

        # 学习: epoch e 错误 → 最终正确
        learned = (~correct_matrix[e]) & final_correct
        L[e] = learned.sum() / n_samples

    # 每个样本被遗忘的次数（在多少个 epoch 正确但最终错误）
...
def load_metrics_csv(log_dir):
    """加载 metrics.csv 获取 val_top1 和 alpha_t"""
...
def plot_forgetting_curves(F, L, acc, epochs, metrics_epochs, val_top1, alpha_t, output_dir):
    ...
    # 图3: Per-class forgetting
    # Note: 需要加载验证集标签才能按类别分析
    # 这里生成占位图
...
def compute_class_forgetting(correct_matrix, val_targets, num_classes=100):
    ...
    for c in range(num_classes):
        ...
        if class_n_samples[c] > 0:
            ...
            if n_wrong > 0:
                # 该类中被遗忘的样本在"曾正确"的比例
                forgotten_in_class = correct_matrix[:, wrong_final_in_class].any(axis=0).sum()
...
def generate_report(F, L, acc, epochs, forgotten_samples, dF, output_dir):
    ...
    with open(report_path, 'w', encoding='utf-8') as f:
        ...
        f.write(f"训练总轮数: {len(epochs)}\n")
        f.write(f"验证集样本数: {len(acc)}\n\n")

        f.write(f"--- 全局指标 ---\n")
        f.write(f"初始验证精度 (epoch {epochs[0]}): {acc[0]*100:.2f}%\n")
        f.write(f"最终验证精度 (epoch {epochs[-1]}): {acc[-1]*100:.2f}%\n")
        f.write(f"最高验证精度: {acc.max()*100:.2f}% (epoch {epochs[np.argmax(acc)]})\n\n")

        f.write(f"--- 遗忘指标 ---\n")
        f.write(f"最大 Forget Fraction: {F.max()*100:.2f}% (epoch {epochs[np.argmax(F)]})\n")
        f.write(f"最终 Forget Fraction: {F[-1]*100:.2f}%\n")
        f.write(f"最终 Learn Fraction: {L[-1]*100:.2f}%\n")
        f.write(f"净遗忘 (F-L at final epoch): {(F[-1]-L[-1])*100:.2f}%\n")
        f.write(f"总 Churn (F+L at final epoch): {(F[-1]+L[-1])*100:.2f}%\n\n")

        f.write(f"--- 遗忘动力学 ---\n")
        f.write(f"遗忘速率最大值: {dF.max()*100:.4f}%/epoch (epoch {epochs[1:][np.argmax(dF)]})\n")

        # 找出遗忘加速的转折点
        mid_point = len(dF) // 2
        early_rate = np.mean(dF[:mid_point])
        late_rate = np.mean(dF[mid_point:])
        f.write(f"早期遗忘速率 (前50%): {early_rate*100:.4f}%/epoch\n")
        f.write(f"后期遗忘速率 (后50%): {late_rate*100:.4f}%/epoch\n")
...
def main():
    ...
    # 3. 识别被遗忘样本
    print("[3/5] Identifying most-forgotten samples...")
    ...
    # 5. 生成图表和报告
    print("[5/5] Generating plots and report...")
    dF = compute_forgetting_rate(F, epochs)
    ...
    # 保存被遗忘样本列表
    forgotten_csv = os.path.join(output_dir, 'forgotten_samples.csv')
