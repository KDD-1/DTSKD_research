...
============================================================================
基于 Wu et al. (ICML 2026) 的理论:
  - 信号遗忘 = 遗忘曾经正确的伪标签 → 有害
  - 去噪     = 遗忘不正确的伪标签 → 有益

操作化定义 (从 per-epoch correctness 矩阵推断):
  有害遗忘样本:
    - 连续正确很多 epoch → 突然被遗忘 → 不再恢复
    - 遗忘发生在训练后期 (模型已稳定)
    - 高稳定性 (correct span 长, 波动少)
...
def classify_forgetting(cm, eps):
    """
    对每个最终错误的样本, 判断是"有害遗忘"还是"有益去噪"

    特征:
      - stability: 最长连续正确段长度 / 总 epoch 数
      - late_forget: 是否在训练后半段 (epoch > 100) 才被遗忘
      - oscillation: 正确→错误→正确的翻转次数
      - first_correct: 首次正确的 epoch
...
print("Loading data...")
cm_hist, eps_hist = load_correct_matrix('experiments/forgetting_hist_s27')
cm_ours, eps_ours = load_correct_matrix('experiments/forgetting_ours_s27')
...
print(f"\n{'='*60}")
print(f"HistKD 最终错误样本: {len(hist_forgotten)}")
...
print(f"  阈值 (median harmful_score): {median_score:.4f}")
print(f"{'='*60}")
...
print(f"有害遗忘样本中 Ours 救回的: {ours_rescued_harmful}/{len(harmful_indices)} "
      f"({ours_rescued_harmful/len(harmful_indices)*100:.1f}%)")
print(f"有益去噪样本中 Ours 救回的: {ours_rescued_benign}/{len(benign_indices)} "
